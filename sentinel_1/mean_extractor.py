...
class Correction_dict(object):
    def __init__(self, safe_dir, shape, tmp_dir, example_dir, gpt_path):
    ...
    def log_to_mean_dict(self, safe_file, mean_dict):
        ...
        for band_index in range(1, gdal_safe.RasterCount + 1):
            ...
            filename = (
                os.path.basename(safe_file).replace(".zip", "_")
                + band_info
                + "_band.tif"
            )
            ...
            snap_executor.graph_processing(
                safe_dir, output_geotiff, "land_sea_mask.xml", ".tif", ".tif"
            )

            assert (
                filename not in mean_dict
            ), f"## {filename} already in dict! Check for duplicate input files"
            mean_dict[filename] = self.get_mean_from_shape_extent(
                output_geotiff, "input/CLIP_TEST_OUT.tif"
            )
